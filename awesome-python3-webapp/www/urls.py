...
from apis import api,Page,APIError,APIValueError,APIPermeissionError,APIResourceNotFoundError
...
def check_admin():
	...
	if user and user.admin:
		return 
	raise APIPermeissionError('No permission.')
...
def register_user():
	...
	if not email or not _RE_EMAIL.match(password):
...
def api_create_blog_comment(blog_id):
	...
	c = Commment(blog_id = blog_id,user_id = user.id,user_name = user.name,user_image=user.image,content = content)
	c.insert()
