...
_RESPONSE_HEADER_DICT = dict(zip(map(lambda x:x.upper(),_RE_RESPONSE_HEADERS),_RE_RESPONSE_HEADERS))
...
class HttpError(Exception):
	...
	def __init__(self,code):
		'''
		Init an HttpError with resposne code.
		'''
	...
	def header(self,name,value):
		...
		self._headers.append((name,values))
...
class Request(object):
    ...
    def get(self,key,default = None):
    	'''
    	The same as request[key] ,but return default value if key not found 
    ...
    def input(self,**kw):
    	'''
    	get input as dict from request,file dict using provided if key not exist
    ...
    def document_root(self):
    	'''
    	Get raw document_root for str.Return '' if no document_root
	...
	def headers(self):
		'''
		Get all HTTP headers with key as str and value as unicode .The header names are 'XXX-XX' uppercase 
	...
	def header(self,header,default=None):
		'''
		Get header from quest as unicode , return None if not exists, or default if specified.
    ...
    def cookie(self,name,default = None):
    	...
    	>>> r = Request({'HTTP_COOKIE':'A=123;url = url=http%3A%2F%2Fwww.example.com%2F'})
    	>>> r.cookie('A')
    	u'123'
    	>>> r.cookie('url')
    	u'http://www.example.com/'
...
class Response(object):
    ...
    def unset_header(self,name):
    	'''
    	Unset header bu name and value.
	...
	def set_cookie(self,name,value,max_age=None,expires=None,path='/',domain=None,secure=False,http_only=True):
		...
		Args:
			...
			expires:optional,unix timestamp,dattime or date object that indicate an absolute time of the expiration time of cookie.
					Note that if expires specified,the max_age will be ignored.
			path:the cookie path,default to '/'.
			domain:the cookie domain,default to '/'.
...
class WSGIApplication(object):
	...
	def run(self,port = 9000,host='127.0.0.1'):
		...
		server = make_server(host,port,self.get_wsgi_applicatioin(debug = True))
	...
	def get_wsgi_application(self,debug=False):
		...
		def wsgi(env,start_response):
			...
			response = ctx.response = Resposne()
			try:
				r = fn_exec()
